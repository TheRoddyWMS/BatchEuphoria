...
import de.dkfz.execution.jobs.GenericJobInfo
import de.dkfz.execution.jobs.Job
import de.dkfz.execution.jobs.JobDependencyID
import de.dkfz.execution.jobs.JobResult
import de.dkfz.execution.jobs.JobState
import de.dkfz.execution.jobs.ProcessingCommands
import de.dkfz.execution.jobs.ReadOutJob
...
//                    }
    ...
    public PBSCommand createCommand(GenericJobInfo jobInfo) {
        Job job = new Job(jobInfo.getJobName(), jobInfo.getParameters(),null, new LinkedHashMap<String, Object>(jobInfo.getParameters()),null);
        PBSCommand pbsCommand = new PBSCommand(job, job.getJobName(), null, job.getParameters(), null, jobInfo.getParentJobIDs(), null,null, null);
        return pbsCommand;
    }

    @Override
    public PBSCommand createCommand(Job job, String jobName, List<ProcessingCommands> processingCommands, File tool, Map<String, String> parameters, List<String> dependencies, List<String> arraySettings) {
        PBSCommand pbsCommand = new PBSCommand(job, jobName, processingCommands, parameters, arraySettings, dependencies,null, null, null );
        return pbsCommand;
    }
    ...
    public ProcessingCommands convertResourceSet(AppConfig configuration, ResourceSet resourceSet) {
        ...
        if (resourceSet.isCoresSet() || resourceSet.isNodesSet()) {
            ...
            String enforceSubmissionNodes = configuration.getProperty(CVALUE_ENFORCE_SUBMISSION_TO_NODES, null);
...
//            sb.append(" -l mem=").append(resourceSet.getMem()).append("g");
    ...
    public ProcessingCommands extractProcessingCommandsFromToolScript(File file) {
        String[] text = IOHelperMethods.loadTextFile(file);
...
//        #PBS -m a
    ...
    @Override
    public Job parseToJob(String commandString, List<Job>parentJobs) {

        GenericJobInfo jInfo = parseGenericJobInfo(commandString);
        Job job = new ReadOutJob(jInfo.getJobName(), jInfo.getID(), jInfo.getParameters(), parentJobs);
    ...
    @Override
    public GenericJobInfo parseGenericJobInfo(String commandString) {
        ...
        for (String option : options) {
            ...
            if (var.equals("N")) {
                ...
                String[] variables = parms.split(StringConstants.SPLIT_COMMA);
                for (String variable : variables) {
                    String[] varSplit = variable.split(StringConstants.SPLIT_EQUALS);
...
//                    println deps[1 .. -1]
        ...
        GenericJobInfo jInfo = new GenericJobInfo(jobName, toolID, id, parameters, dependencies);
        jInfo.setCpus(cores);
...
//        jInfo.set
    ...
    public JobResult convertToArrayResult(Job arrayChildJob, JobResult parentJobsResult, int arrayIndex) {
        ...
        return new JobResult(parentJobsResult.getCommand(), pjid, parentJobsResult.isWasExecuted(), false, parentJobsResult.getToolID(), parentJobsResult.getJobParameters(), parentJobsResult.getParentFiles());
    }
    ...
    abstract protected void updateJobStatus(boolean forceUpdate)
    ...
    @Override
    public void queryJobAbortion(List<Job> executedJobs, ExecutionService executionService) {
